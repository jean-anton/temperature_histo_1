...
class HourlyChartBuilder {
    ...
    if (forecast.dailyForecasts.isNotEmpty) {
      ...
      if (firstDay.sunrise != null) {
        ...
        if (startTime.isBefore(sunriseTime) && sunriseTime.isAfter(startTime)) {
          ...
          final sunriseScreenPos = ChartHelpers.calculateScreenPosition2(
            sunriseTime.millisecondsSinceEpoch.toDouble(),
            maxTemp, // Y coordinate doesn't matter for X position calculation
            containerSize,
            minTemp,
            maxTemp,
            startTime,
            ...
            Positioned(
              ...
              child: Container(
                color: Colors.blue.withOpacity(
                  0.2,
                ), // Semi-transparent blue for night
                child: null,
              ),
            ),
          );
    ...
    for (var dailyForecast in forecast.dailyForecasts) {
      ...
      if (dailyForecast.sunset != null && dailyForecast.sunrise != null) {
        ...
        if (sunsetTime.isAfter(startTime) &&
          ...
          final sunsetScreenPos = ChartHelpers.calculateScreenPosition2(
            sunsetTime.millisecondsSinceEpoch.toDouble(),
            maxTemp, // Y coordinate doesn't matter for X position calculation
            containerSize,
            minTemp,
            maxTemp,
  ...
  static List<Widget> _buildWeatherIcons(
    ...
    return hourlyWeather.hourlyForecasts.asMap().entries.map((entry) {
      ...
      var screenPosition = ChartHelpers.calculateScreenPosition2(
